...
# At Slurm job allocation time, the HPC scripts create /tmp/scratch/<job-id>, which is writable.
# Those scripts export the environment variable TMPDIR with that value, but this worker script
# won't have access to it because it runs in an ssh session.
# Also, we may be creating a Spark cluster that spans multiple Slurm jobs, and so there is no way
